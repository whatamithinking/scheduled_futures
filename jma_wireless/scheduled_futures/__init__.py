    ...
    def __init__(
        ...
        if self._weak is None:
            if (
                ...
                or isinstance(fn, LambdaType)
                or isinstance(fn, weakref.ref)
            ):
                self._weak = False
            else:
                self._weak = True
    ...
    def run(self):
        ...
        if isinstance(self._fn, weakref.ref):
            ...
            if func is None:
                if self.is_periodic():
                    msg = (
                        "Periodic future lost reference to callable and will "
        ...
        try:
            ...
            if run_lag > self._pool.late_run_limit:
                log.warning(
                    "Late to run scheduled future.",
                    extra=dict(
                        metadata=dict(
                            run_lag=run_lag,
                            late_run_limit=self._pool.late_run_limit,
                            future_name=self._name,
                            future_id=self._id,
                        )
                    ),
                )
            ...
            if self.is_periodic():
                log.exception(
                    "Periodic future raised exception.",
                    extra=dict(
                        metadata=dict(future_name=self._name, future_id=self._id)
                    ),
                )
        ...
        with self._condition:
            ...
            if self.is_periodic() and runtime > self.period():
                log.warning(
                    "Periodic scheduled future runtime exceeded period.",
                    extra=dict(
                        metadata=dict(
                            runtime=runtime,
                            period=self.period(),
...
class ScheduledThreadPoolExecutor(ThreadPoolExecutor):
    def __init__(
        self, late_run_limit: float = 1, daemon: bool = False, *args, **kwargs
    ):
